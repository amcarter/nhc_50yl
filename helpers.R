...
drift_correct <-  function(yd,  colname1, colname2){
  ...
  for(i in 1:(nrow(ysi_pts)-1)){
    ...
    if(!is.na(meas_p1)){ # the first point is a measurement
      if(is.na(sens_p1)) next
      if(!is.na(meas_p2)){ # the second point is a measurement
      ...
      if(!is.na(sens_p2)){ # the second point is data
...
# Plot two things using dygraphs
plot_drift_corr <- function(NHC, waterpres = "level_m", airpres = "waterdepth_m",
                      extra = NA){
    if(is.na(extra)){
