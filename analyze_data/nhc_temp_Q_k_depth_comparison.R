...
hall_stages <- read_csv('data/hall/hall_table_13_p.csv') %>%
  mutate(level_meas = water_stage_cm/100) %>%
  filter(site == 'Concrete') %>%
  select(date, level_meas)
hall_stages <- 
  read_csv('data/hall/hall_table_5_flood_stages_cbp.csv') %>%
...
hall_dat <- hall_met %>% 
  ...
  arrange(date)
write_csv(hall_dat, 'data/hall_hall_discharge_temp_daily_corrected_dates.csv')
# add K values ####
...
cbp <- read_csv("data/metabolism/processed/CBP.csv")
# met <- read_csv("data/metabolism/compiled/daily_preds_stream_metabolizer.csv") %>% # I don't know where this file was generated. Figure it out!
...
tiff(filename = "figures/SI/disch_depth_K600_distributions_CBP.tif", 
    width = 6, height = 2.5, units = 'in', res = 800, compression = 'lzw')
  ...
  plot(density(log(today$discharge), na.rm = T), ylim = c(0, 0.45), 
       xlim = c(-5, 5), main = "Discharge", yaxt = 'n',
       xlab = "log discharge (m3s)")
  ...
  plot(density(today$K600, na.rm = T), main = "Gas Exchange", 
       xlab = "K600 (d-1)", yaxt = 'n', ylim = c(0,.13))
