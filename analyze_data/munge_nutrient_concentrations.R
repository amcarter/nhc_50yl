...
dat_mar <- read_csv("data/longitudinal_sampling/NHCLongitudinalDO_20190308.csv") %>%
  mutate(depth_cm = as.numeric(depth_cm), 
         NO3.N_mgl = NO3.N_mgl/10)
dat_oct <- read_csv("data/longitudinal_sampling/NHCLongitudinalDO_20191009.csv")
...
dat <- read_csv('data/rating_curves/interpolatedQ_allsites.csv') %>%
