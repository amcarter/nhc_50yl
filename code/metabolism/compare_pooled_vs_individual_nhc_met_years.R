...
filelist <- list.files("data/metabolism/modeled/finalQ")[2:4]
GPP_min = 0
ER_max = 0
...
fit_pooled <- readRDS("data/metabolism/modeled/fit_nhc_uninformed_raymond_K.rds")
obserr <- get_fit(fit_pooled)$overall %>%
    select(err_obs_iid_sigma_50pct, err_obs_iid_sigma_Rhat,
           err_proc_iid_sigma_50pct, err_proc_iid_sigma_Rhat) %>%
    mutate(site = 'nhc_pool')

get_fit(fit_pooled)$overall %>% select(ends_with('Rhat')) %>%glimpse()
get_fit(fit_pooled)$KQ_overall %>% glimpse()
