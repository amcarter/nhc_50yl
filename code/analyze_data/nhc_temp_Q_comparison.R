...
nhc <- readRDS("data/metabolism/compiled/met_preds_stream_metabolizer_O2.rds")$filled %>%
  filter(site == "NHC")

nhc_q <- read_csv('data/rating_curves/interpolatedQ_allsites.csv') %>%
    mutate(date = as.Date(DateTime_UTC)) %>%
    group_by(date) %>%
    summarize(discharge = mean(NHC.Q, na.rm = T))
nhc_qt <- nhc %>%
    select(-discharge) %>%
    left_join(nhc_q, by = 'date') %>%
  group_by(doy) %>%
...
hall <- read_csv("data/hall_data/hall_discharge_temp_daily.csv") %>%
...
h <- hall %>% group_by(month(date)) %>%
    mutate(across(-notes, ~mean(., na.rm = T)))
# nhc %>% group_by(month(date)) %>%
#     mutate(across(-notes, ~mean(., na.rm = T)))
#   select('month(date)', temp_now = temp.water) %>%
#   full_join(h, by = 'month(date)') %>%
#   mutate(temp_diff = temp_now - temp.water)

png("figures/watertemp_comparison.png", width = 3, height = 2.5,
    units = 'in', res = 800)
...
par(ps = 10,
    ...
    mar = c(0.5, 3, 0.5, 0.5),
    oma = c(1.5,0,0,0))
plot(nhc_qt$doy, nhc_qt$temp.water, type = 'l', ylim = c(0, 30),
   xaxt = 'n', yaxt='n')
axis(2, at = seq(0,30, by = 15), cex.axis = axis_size, las = 2, labels = FALSE,
   tcl = -0.3)
mtext(side = 2, line = 0.6, at = seq(0,30, by = 15), las = 2,
    text = seq(0,30, by = 15), adj = 1, cex = axis_size)
mtext('Water temp (°C)', 2, cex = 0.8, line = 1.7)
...
lines(nhc_qt$doy, nhc_qt$temp.water)
lines(hall_qt$doy, hall_qt$temp.water, col = then_col)
legend('topleft', legend = c(2019, 1969), col = c(1, then_col),
     lty = 1, ncol = 1, bty = 'n', cex = .65, seg.len = .75, x.intersp = 0.5)

plot(nhc_qt$doy, nhc_qt$discharge, log = 'y', xaxt = 'n',
   type = 'l', yaxt = 'n', cex.axis = axis_size,
   ylim = c(0.01, 500))
   # ylim = range(c(nhc_qt$discharge_min, nhc_qt$discharge_max)))
mtext(expression('Discharge (m'^3 * s^-1 *')'), 2, 1.7, cex = 0.8)
polygon(c(nhc_qt$doy, rev(nhc_qt$doy)),
...
polygon(c(hall_qt$doy, rev(hall_qt$doy)),
      ...
      col = alpha(then_col, .3), border = NA)
lines(nhc_qt$doy, nhc_qt$discharge)
lines(hall_qt$doy, hall_qt$discharge, col = then_col)
axis(1, at = as.numeric(format(seq(as.Date('2020-01-01'),
                                 by = 'month', length.out = 12),'%j')),
   labels = substr(month.abb, 1,1), cex.axis = axis_size, las = 1,
   padj = -2, tcl = -0.3)
axis(2, at = c(0.01, .1, 1, 10, 100), cex.axis = axis_size,
   las = 2, labels = FALSE, , tcl = -0.3)
mtext(side = 2, line = 0.6, at = c(0.01, .1, 1, 10, 100), las = 2,
    text = c(0.01, .1, 1, 10, 100), adj = 1, cex = axis_size)

dev.off()


tiff("figures/watertemp_comparison.tif", width = 3, height = 2.25,
    units = 'in', res = 800)
...
write_csv(Q_yearly, 'data/rating_curves/annual_Q_stats.csv')
bf_19 <- Q_stats %>%
  select(year, bf_frac) %>%
  group_by(year)
bf_69 <- Q %>%
