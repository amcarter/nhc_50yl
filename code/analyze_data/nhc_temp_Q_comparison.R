...
nhc <- readRDS("data/metabolism/compiled/met_preds_stream_metabolizer.rds")$filled %>%
  filter(site == "NHC") 
...
hall <- read_csv("data/hall/hall_discharge_temp_daily.csv") %>%
...
tiff("figures/watertemp_comparison.tif", width = 3, height = 2.25, 
 ...
 par(ps = 10,
  ...
  plot(nhc_qt$doy, nhc_qt$temp.water, type = 'l', ylim = c(0, 30),
       xaxt = 'n', yaxt='n')
  axis(2, at = seq(0,30, by = 15), cex.axis = axis_size)
  ...
  mtext('Discharge (m3/s)',2,1.7, cex = 0.8) 
  polygon(c(nhc_qt$doy, rev(nhc_qt$doy)),
  ...
  axis(1, at = as.numeric(format(seq(as.Date('2020-01-01'), 
       ...
       labels = substr(month.abb, 1,1), cex.axis = axis_size, las = 2) 
  axis(2, at = c(.01, 1, 100), labels = c(0.01, 1, 100), cex.axis = axis_size)
