...
# a. download
source('code/download_format_data/download_rawSPdata.R')

# b. correct level data
source("code/download_format_data/correct_level_data.R")

# c. gap fill level data at NHC and UNHC
source("code/download_format_data/gap_fill_level.R")

# d. Build rating curves for NHC and UNHC
#    Calculate discharge, interpolate between sites based on watershed area
source("code/download_format_data/update_rating_curves.R")

# e. Calculate coefficients for depthxQ and velocityxQ relationships
source("code/download_format_data/calc_avg_channel_crosssections.R")

# f. get light data from StreamLight package:
source("code/download_format_data/get_light_from_StreamLight.R")

# g. Prepare datafiles for modeling metabolism
source("code/download_format_data/prep_raw_SPdata.R")
...
# a. Load and compile metabolism runs from SM
  source("code/metabolism/compile_sm_models.R")
...
sites <- sites[c(1:5,7),]
  ...
  source("code/analyze_data/bootstrap_metabolism_comparison_streamMetabolizer.R") #**
  ...
  source("code/plot_data/plot_direct_calc_met_results.R")
  # plot temp x ER and GPP relationships
  # this script generates figures 3, 4
  source("code/analyze_data/calculate_Q10_SM_plot.R")
...
# Calculations and data processing:
  ...
  source('code/analyze_data/nhc_temp_Q_k_depth_comparison.R')


################################################################################
# Other plots:
  # plot temperature and discharge comparison
  source("code/analyze_data/nhc_temp_Q_comparison.R")
  # plot land cover and terrestrial metabolism
  # source("code/plot_data/plot_riparian_NPP_LAI.R") # not this one
  # source("code/plots/plot_watershed_landcover.R")
  ...
  # summarize temperature, discharge, and DO across all metabolism site years
    ...
    source('code/analyze_data/summarize_physical_met_drivers.R')

  # summarize hall and todays nutrient chemistry
    source('code/analyze_data/munge_nutrient_concentrations.R')
# plot multipanel climate figure
  source('code/plot_data/multi_panel_watershed_change_fig.R')
